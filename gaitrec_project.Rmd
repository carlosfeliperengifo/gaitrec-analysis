...
# Results
This section presents four models: (i) k-nearest neighbors and (ii) random 
forest, both trained with the principal components of the GRF (48-predictors), 
and (iii) k-nearest neighbors and (iv) random forest trained with the mean, 
standard deviation, and maximum of GRF in each direction (6-predictor).
...
musculoskeletal disorder (A, C, H, or K), a participant is likely to be classified
...
### Random forest model
The following code calculates and validates a random forest model. In the sake 
of computation time *Rborist* was used instead of *train*
...
The confusion matrix above shows that the random forest model performs as poorly 
as the k-nearest neighbors model. The random forest, despite its complexity, 
increased the overall accuracy only from `r acc_knn` to `r acc_rf`.
...
The confusion matrix shows that the specificities of the k-nearest neighbors 
model trained with simple statistics are higher than those of the model trained 
with PCA. The greatest improvement in specificity is obtained for the HC class.
...
### Random forest model
The following code calculates and validates a random forest model whose training
...
The confusion matrix shows that the specificities of the random forest model 
trained with simple statistics outperform the models trained with PCA. Like 
k-nearest neighbors models, the greatest improvement in specificity is obtained 
for the HC class. 
